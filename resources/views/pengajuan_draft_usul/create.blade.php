    ...
    <div class="card-body">
        ...
        <form action="{{route('tugas_akhir.pengajuan_draft_usul.simpan')}}" method="POST">
            ...
            <div class="card">
                <div class="card-header bg-success">
                    <h3 class="card-title"><i class="fas fa-history"></i> Detail Data Draft Ajuan Usul Penelitian</h3>
                    @if($data->stat_ajuan ==0)
                ...
                <div class="card-body">
                    ...
                    {!! FormInputTextarea('keterangan','Keterangan') !!}
                </div>
            </div>
        </form>
        @endif
        ...
        <div class="card-footer">
            ...
            <div class="pull-right">
                @if($data->stat_ajuan==0)
                <form action="{{ route('tugas_akhir.pengajuan_draft_usul.simpan_permanen_ajuan', Crypt::encrypt()) }}" class="validasi_form" style="display: inline;" method="POST">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="id_ajuan_draft_usul" value="{{ $data->id_ajuan_draft_usul }}">
                ...
                <form action="{{ route('tugas_akhir.pengajuan_draft_usul.delete',Crypt::encrypt($data->id_ajuan_draft_usul)) }}" class="validasi_form" style="display: inline;" method="POST">
                    @csrf
                    @method('DELETE')
                    <button class="btn btn-xs btn-danger btn-flat btn-validasi"><i class="fa fa-refresh"></i> Tarik Kembali dan Perbaiki Pengajuan Draft Usul Penellitian</button>
                </form>
                @push('js')
                <script>
                    $(document).ready(function() {
