...
<div class="card">
    ...
    <div class="card-body">
        ...
        @if ($data->status_validasi == 3 && $data_nilai->a_valid == 0)

        <form action="{{ route('validasi.pengajuan_seminar.update_jadwal_seminar', Crypt::encrypt($data->id_daftar_seminar)) }}"
            method="POST">
            @csrf
            @method('PUT')
            <div class="card">
                <div class="card-header bg-success">
                    <h3 class="card-title"><i class="fas fa-list-alt"></i> Jadwal Seminar {{ $seminar->jenisSeminar->nm_jns_seminar }} - {{ $profil->nm_pd }}</h3>
                </div>
                <div class="card-body">
                    ...
                    @if (in_array($data->status_validasi, [3]))
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <form action="{{ route('validasi.pengajuan_seminar.ubah',Crypt::encrypt($data->id_daftar_seminar)) }}" method="POST">
                                @csrf
                                @method('PUT')
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <a href="{{ route('validasi.pengajuan_seminar.detail.daftar_riwayat_verifikasi', [Crypt::encrypt($data->id_daftar_seminar), Crypt::encrypt($each_syarat->id_list_syarat)]) }}"
        ...
        <div class="card">
            ...
            <form class="validasi_form" action="{{ route('validasi.pengajuan_seminar.update',Crypt::encrypt($data->id_daftar_seminar)) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <input type="hidden" name="nm_verifikator" value="{{ auth()->user()->nm_pengguna }}">
                    {!! FormInputSelect('status_validasi','Status Periksa',true,false,config('mp.data_master.status_ajuan_seminar_admin'),$data->status_validasi) !!}
            ...
            <div class="card-body" style="margin: 0; padding: 0">
                <table class="table table-striped">
                    <tbody>
                        ...
                        @php
                        $noBeritaAcara = $no_ba_seminar
                        ? ($no_ba_seminar->no_ba_daftar_seminar . '/' . $no_ba_seminar->kode_ba_daftar_seminar)
                        : '-';
                        @endphp

                        {!! tableRow('No. Berita Acara Seminar', $noBeritaAcara) !!}
                        @endif
                    </tbody>
                </table>
            </div>
